...
export class ConversationService {
  ...
  handleNavigationToConversation(userPublicId: string) {
  ...
  handleGetAll(pagination: Pagination) {
    ...
    this.httpClient.get<Array<Conversation>>(`${environment.API_URL}/conversations`, {
      params
    }).subscribe({
      next: conversations => {
  ...
  private mapDateToDayJs(conversation: Conversation) {
    ...
    conversation.members
        .forEach(member => member.lastSeen = dayjs(member.lastSeen));
  }
  ...
  sortConversationByLastMessage(conversations: Array<Conversation>) {
    conversations.sort((conversationA, conversationB) => {
      if (conversationA.messages && conversationB.messages &&
        conversationA.messages.length != 0 && conversationB.messages.length != 0) {
  ...
  handleCreate(conversationToCreate: ConversationToCreate) {
    this.httpClient.post<Conversation>(`${environment.API_URL}/conversations`, conversationToCreate)
        .subscribe({
          next: conversation => {
  ...
  handleDelete(conversationPublicId: string) {
    ...
    this.httpClient.delete<string>(`${environment.API_URL}/conversations`, {params})
        .subscribe({
  ...
  handleGetOne(conversationId: string) {
    ...
    this.httpClient.get<Conversation>(`${environment.API_URL}/conversations/get-one-by-public-id`, {params})
        .subscribe({
          next: conversation => {
            this.mapDateToDayJs(conversation);
  ...
  getReceiverMember(conversation: Conversation) {
  ...
  handleMarkAsRead(conversationId: string): void {
    const params = new HttpParams().set("conversationId", conversationId);
    this.httpClient.post<number>(`${environment.API_URL}/conversations/mark-as-read`, {}, {params})
        .subscribe({
