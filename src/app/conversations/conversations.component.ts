...
export class ConversationsComponent implements OnInit, OnDestroy{
  ...
  oauth2Service: Oauth2AuthService = inject(Oauth2AuthService);
  ...
  private listenToGetAllConversation() {
  ...
  private listenToSSEDeleteConversation() {
    this.deleteSSESub = this.sseService.deleteConversation.subscribe(uuidDeleted => {
      const indexToDelete = this.conversations
          .findIndex(conversation => conversation.publicId === uuidDeleted);
      this.conversations.splice(indexToDelete, 1);
      this.toastService.show("Conversation deleted by the user", "SUCCESS");
    })
  }

  private listenToSSENewMessage() {
    this.sseService.receiveNewMessage.subscribe(newMessage => {
      const indexToUpdate = this.conversations
          .findIndex(conversation => conversation.publicId === newMessage.conversationId);
      if (indexToUpdate === -1) {
